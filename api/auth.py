...
class PasswordAuthMiddleware(BaseHTTPMiddleware):
    ...
    Always active with default password if VAULT_PASSWORD is not set.
...
def check_api_password(
    ...
    Supports Docker secrets via VAULT_PASSWORD_FILE.
    Returns True without checking credentials if VAULT_PASSWORD is not configured.
    Raises 401 if credentials are missing or don't match the configured password.
    """
    password = get_secret_from_env("VAULT_PASSWORD")
