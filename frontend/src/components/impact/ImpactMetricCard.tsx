...
export function ImpactMetricCard({ label, value, suffix = '', note, href, icon: Icon, progress, tone = 'gold' }: { label: string; value: number; suffix?: string; note: string; href: string; icon: ComponentType<{ className?: string }>; progress?: number; tone?: 'gold' | 'cyan' | 'blue' | 'violet' }) {
  ...
  return <motion.div whileHover={reduce ? undefined : { y: -3 }}><Card className="h-full gap-0 rounded-2xl border-slate-200 p-4 shadow-sm transition-shadow hover:shadow-md">
    <div className="flex items-start justify-between"><span className={`grid size-10 place-items-center rounded-xl ${tones[tone]}`}><Icon className="size-5" /></span><a href={href} aria-label={`View ${label}`} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ArrowRight className="size-4" /></a></div>
