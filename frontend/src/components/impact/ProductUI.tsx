...
export function PageHeader({ eyebrow, title, description, actions, breadcrumbs }: { eyebrow?: string; title: string; description: string; actions?: ReactNode; breadcrumbs?: { label: string; href?: string }[] }) {
  return (
    <header className="mb-8 rounded-2xl border border-[#1e293b] bg-[#0E1626] px-5 py-6 shadow-[0_8px_24px_rgba(2,6,23,0.28)] sm:px-6 sm:py-7">
      {breadcrumbs && (
        <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400">
          {breadcrumbs.map((item, index) => (
            <span key={`${item.label}-${index}`} className="flex items-center gap-2">
              {index > 0 && <span aria-hidden="true" className="text-slate-400">/</span>}
              {item.href ? <a href={item.href} className="rounded text-slate-400 hover:text-cyan-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500">{item.label}</a> : <span aria-current="page" className="text-slate-200">{item.label}</span>}
            </span>
          ))}
        </nav>
      )}
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-3xl">
          {eyebrow && <p className="mb-2 text-xs font-extrabold uppercase tracking-[0.16em] text-[#f5c542]">{eyebrow}</p>}
          <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">{title}</h1>
          <p className="mt-3 max-w-[70ch] text-base leading-7 text-slate-400">{description}</p>
        </div>
...
export function SecondaryAction({ href, children, download }: { href: string; children: ReactNode; download?: string }) {
  return <a href={href} download={download} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-[#1e293b] bg-[#0E1626] px-5 py-2.5 text-sm font-bold text-slate-200 shadow-sm transition-colors hover:border-[#f5c542]/40 hover:text-[#f5c542] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500">{children}</a>
}
...
export function MetricCard({ label, value, detail, icon: Icon, tone = 'teal' }: { label: string; value: string | number; detail?: string; icon: LucideIcon; tone?: 'teal' | 'amber' | 'blue' | 'violet' }) {
  const tones = {
    teal: 'bg-cyan-400/10 text-cyan-300 border-cyan-400/20',
    amber: 'bg-[#f5c542]/10 text-[#f5c542] border-[#f5c542]/20',
  ...
  return (
    <article className="rounded-2xl border border-[#f5c542]/25 bg-[#0E1626] p-5 shadow-[0_8px_24px_rgba(2,6,23,0.28)] sm:p-6">
      ...
      <p className={`mt-2 text-4xl font-black tracking-[-0.04em] sm:text-5xl ${tone === 'amber' ? 'text-[#f5c542]' : 'text-white'}`}>{value}</p>
      {showProgress && <div className="mt-4 h-3 overflow-hidden rounded-full bg-white/10" role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><div className={`h-full rounded-full ${tone === 'amber' ? 'bg-[#f5c542]' : 'bg-cyan-500'}`} style={{ width: `${progress}%` }} /></div>}
...
export function SectionCard({ title, description, action, children, className = '' }: { title: string; description?: string; action?: ReactNode; children: ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-[#1e293b] bg-[#0E1626] shadow-[0_8px_24px_rgba(2,6,23,0.28)] ${className}`}>
      <div className="flex flex-col gap-3 border-b border-[#1e293b] p-6 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-xl font-bold tracking-tight text-white sm:text-2xl">{title}</h2>{description && <p className="mt-2 text-sm leading-6 text-slate-400">{description}</p>}</div>
        {action}
      </div>
...
export function StatusBadge({ children, tone = 'neutral' }: { children: ReactNode; tone?: 'success' | 'attention' | 'info' | 'neutral' }) {
  const tones = { success: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300', attention: 'border-amber-500/30 bg-amber-500/10 text-amber-300', info: 'border-cyan-500/30 bg-cyan-500/10 text-cyan-300', neutral: 'border-slate-500/30 bg-white/10 text-slate-300' }
...
export function ProgressBar({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="font-medium text-slate-400">{label}</span><span className="font-bold text-slate-200">{safe}%</span></div>
      <div className="h-3 overflow-hidden rounded-full bg-white/10" role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={safe}><div className="h-full rounded-full bg-[#f5c542]" style={{ width: `${safe}%` }} /></div>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between gap-3 text-xs">
        ...
        <span className="font-bold text-slate-200">{showValue ? `${value}%` : ''}{count != null && <span className="ml-1.5 font-normal text-slate-400">n={count}</span>}</span>
      </div>
      <div className="h-2.5 overflow-hidden rounded-full bg-white/10" role="meter" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={value}>
    ...
    <div className="space-y-3">
      {rows.map((row) => (
        <div key={row.rowLabel}>
          <p className="mb-1.5 text-xs font-bold text-slate-300">{row.rowLabel}</p>
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 md:grid-cols-4">
            {row.cells.map((cell) => {
              const color = performanceColor(cell.value)
              return (
                <div key={cell.label} className="rounded-lg border border-[#1e293b] p-2.5" style={{ backgroundColor: `${color}33`, borderLeft: `4px solid ${color}` }}>
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-[11px] font-semibold text-slate-200" title={cell.label}>{cell.label}</span>
                    <span className="shrink-0 text-[12px] font-black" style={{ color }}>{cell.value}%</span>
                  </div>
                  {cell.sub && <p className="mt-0.5 text-[10px] font-medium text-slate-400">{cell.sub}</p>}
                </div>
              )
            })}
          </div>
      ...
      {caption && <p className="text-[11px] leading-4 text-slate-400">{caption}</p>}
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={centerLabel ? `${centerLabel} distribution` : 'distribution'}>
        ...
        {centerLabel && (
          <text x="50%" y="46%" textAnchor="middle" className="fill-white" style={{ fontSize: 20, fontWeight: 900 }}>{centerLabel}</text>
        )}
        {centerSub && (
          <text x="50%" y="60%" textAnchor="middle" className="fill-slate-400" style={{ fontSize: 9, fontWeight: 600 }}>{centerSub}</text>
        )}
      </svg>
      <ul className="space-y-1.5">
        {segments.map((seg) => (
          <li key={seg.label} className="flex items-center gap-2 text-xs">
            ...
            <span className="font-medium text-slate-400">{seg.label}</span>
            <span className="ml-auto font-bold text-white">{seg.value}</span>
          </li>
        ))}
      </ul>
    </div>
          ...
          <div className="flex flex-col gap-1">
            {group.values.map((v, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="w-10 shrink-0 text-right text-[11px] font-bold text-slate-300">{v}</span>
              </div>
            ))}
          </div>
        </div>
...
export function PrePostTrend({ pre, post, label }: { pre: number; post: number; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-xs font-bold text-white">{pre}% → {post}%</p>
...
export function ProductState({ type, title, description, onRetry }: { type: 'loading' | 'empty' | 'error' | 'success'; title: string; description: string; onRetry?: () => void }) {
  ...
  return (
    <div className="grid min-h-56 place-items-center rounded-2xl border border-dashed border-[#1e293b] bg-[#0E1626] p-8 text-center" role={type === 'error' ? 'alert' : 'status'}>
      <div className="max-w-md"><Icon aria-hidden="true" className={`mx-auto h-7 w-7 ${type === 'loading' ? 'animate-spin text-teal-600' : type === 'error' ? 'text-rose-600' : 'text-slate-400'}`} /><h2 className="mt-4 text-base font-extrabold text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{description}</p>{onRetry && <button type="button" onClick={onRetry} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#1e293b] bg-[#0E1626] px-4 py-2 text-sm font-bold text-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"><RotateCcw aria-hidden="true" className="h-4 w-4" />Retry</button>}</div>
    </div>
  )
}
