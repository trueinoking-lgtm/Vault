  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between gap-3 text-xs">
        ...
        <span className="font-bold text-slate-900">{showValue ? `${value}%` : ''}{count != null && <span className="ml-1.5 font-normal text-slate-400">n={count}</span>}</span>
      </div>
      <div className="h-2.5 overflow-hidden rounded-full bg-slate-100" role="meter" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={value}>
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={centerLabel ? `${centerLabel} distribution` : 'distribution'}>
        ...
        {centerLabel && (
          <text x="50%" y="46%" textAnchor="middle" className="fill-slate-950" style={{ fontSize: 20, fontWeight: 900 }}>{centerLabel}</text>
        )}
        {centerSub && (
...
export function CohortBars({ groups, max = 100, sideLabels }: { groups: { label: string; values: number[] }[]; max?: number; sideLabels: string[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {groups.map((group) => (
        <div key={group.label}>
          <p className="mb-1 text-xs font-medium text-slate-600">{group.label}</p>
...
export function PrePostTrend({ pre, post, label }: { pre: number; post: number; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-xs font-bold text-slate-950">{pre}% → {post}%</p>
