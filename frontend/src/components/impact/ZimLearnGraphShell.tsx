...
import { ZIMLEARNGRAPH_NAV_ITEMS } from '@/lib/impact/product-navigation'
  ...
  return (
    <a href="/impact" className="group flex min-w-0 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2">
      ...
      <span className="min-w-0">
        <span className="block truncate text-base font-bold tracking-[-0.02em] text-slate-950">ZimLearnGraph</span>
...
function ProductNavigation({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  return (
    <nav aria-label="ZimLearnGraph primary navigation" className="space-y-1.5">
      {ZIMLEARNGRAPH_NAV_ITEMS.map((item) => {
...
export function ZimLearnGraphShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f6f8f7] text-slate-900">
      <a href="#zg-main" className="sr-only z-[100] rounded-md bg-white px-4 py-3 font-semibold text-slate-950 focus:not-sr-only focus:fixed focus:left-4 focus:top-4">
      ...
      <div className="lg:pl-64">
        ...
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-2.5 sm:px-6 lg:px-8" role="note" aria-label="Demonstration data disclosure">
          <div className="mx-auto flex max-w-[1440px] items-start gap-2 text-xs font-medium leading-5 text-amber-950">
            <ShieldCheck aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" />
            <span>{DEMO_DISCLOSURE}</span>
          </div>
        </div>

        <main id="zg-main" className="mx-auto min-h-[calc(100vh-3rem)] w-full max-w-[1440px] px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
          {children}
        </main>
      </div>
