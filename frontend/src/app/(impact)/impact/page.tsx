...
export default function ImpactOverviewPage() {
  ...
  const learnerCoverage = Math.round(assessed / stats.learners * 100); const passRate = stats.averagePassRate
  ...
  return <motion.div key={period} initial={reduce ? false : { opacity: .75, y: 4, scale: .998 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: reduce ? 0 : .25, ease: 'easeOut' }} className="space-y-5 pb-8">
    ...
    <Section {...animation} aria-label="System metrics" className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <ImpactMetricCard label="Learners" value={stats.learners} note={`${assessed} assessed across latest seeded records`} href="/impact/classes" icon={UsersRound} tone="blue" progress={learnerCoverage} />
      <ImpactMetricCard label="Pass Rate" value={passRate} suffix="%" note="Average school pass rate" href="/impact/reports" icon={BarChart3} tone="cyan" progress={passRate} />
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-12">
      ...
      <Section {...animation} className="xl:col-span-4"><Card className="h-full gap-0 rounded-2xl border-slate-200 shadow-sm transition-transform duration-200 hover:-translate-y-1"><CardHeader><CardTitle className="text-xl">Assessment coverage</CardTitle><p className="text-sm text-slate-500">Unique learners represented in seeded school summaries</p></CardHeader><CardContent><div className="flex items-end gap-2"><strong className="text-4xl tracking-tight">{assessed}</strong><span className="pb-1 text-sm text-slate-500">of {stats.learners} learners</span></div><Progress value={learnerCoverage} className="mt-5 h-3" /><div className="mt-4 flex justify-between text-xs"><span className="font-semibold text-cyan-700">Assessed {assessed}</span><span className="text-slate-500">Not represented {unassessed}</span></div><p className="mt-5 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">Coverage reflects the latest seeded school dashboard totals, not a longitudinal attendance measure.</p></CardContent></Card></Section>
      <Section {...animation} className="xl:col-span-4"><Card className="h-full gap-0 rounded-2xl border-slate-200 shadow-sm transition-transform duration-200 hover:-translate-y-1"><CardHeader><CardTitle className="text-xl">Subject performance</CardTitle><p className="text-sm text-slate-500">Seeded assessment pass rates by subject</p></CardHeader><CardContent className="space-y-5">{subjects.map(s => <div key={s.name}><div className="mb-2 flex justify-between text-sm"><span className="font-semibold text-slate-700">{s.name}</span><strong>{s.rate}%</strong></div><Progress value={s.rate} className="h-2" /><p className="mt-1 text-xs text-slate-400">{s.learners} assessed learners</p></div>)}</CardContent></Card></Section>
