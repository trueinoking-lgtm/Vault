...
export default function InterventionsPage() {
  ...
  return <div><PageHeader eyebrow="Teacher-led support" title="Interventions" description="Eight deterministic support actions are connected to assessment evidence. Seven are active; teachers verify, adapt and decide what to apply." />
    <div className="mb-5 grid grid-cols-2 gap-3 sm:max-w-md"><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">All interventions</p><p className="mt-2 text-3xl font-black text-slate-900">{interventions.length}</p></div><div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-5"><p className="text-xs font-bold uppercase tracking-[0.1em] text-cyan-700">Active</p><p className="mt-2 text-3xl font-black text-cyan-700">{interventions.filter((item) => item.status !== 'completed' && item.status !== 'dismissed').length}</p></div></div>
