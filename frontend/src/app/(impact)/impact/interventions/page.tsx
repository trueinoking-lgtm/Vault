...
export default function InterventionsPage() {
  ...
  return <div><PageHeader eyebrow="Teacher-led support" title="Interventions" description="Eight deterministic support actions are connected to assessment evidence. Seven are active; teachers verify, adapt and decide what to apply." />
    ...
    {interventions.length === 0 ? <ProductState type="empty" title="No interventions available" description="Deterministic support actions appear after assessment marks have been analysed." /> : <div className="space-y-7">{groupedInterventions.map(({ school, interventions: schoolInterventions }) => <section key={school.id} aria-labelledby={`school-${school.id}`}><div className="mb-3 flex items-center gap-3"><h2 id={`school-${school.id}`} className="text-sm font-black text-slate-950">{school.name}</h2><span className="h-px flex-1 bg-slate-200" /><span className="text-xs font-semibold text-slate-500">{schoolInterventions.length} {schoolInterventions.length === 1 ? 'action' : 'actions'}</span></div><div className="space-y-3">{schoolInterventions.map((item) => { const assessment = assessments.find((row) => row.id === item.assessment_id); const cls = classes.find((row) => row.id === item.class_group_id); const topic = SEEDED_TOPICS.topics.find((row) => row.id === item.topic_id); return <article key={item.id} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 lg:grid-cols-[auto_1.4fr_.8fr_.65fr_auto] lg:items-center"><span className="grid h-11 w-11 place-items-center rounded-xl bg-teal-50 text-teal-800"><Target aria-hidden="true" className="h-5 w-5" /></span><div><h3 className="text-sm font-extrabold text-slate-950">{item.recommendation}</h3><p className="mt-1 text-xs leading-5 text-slate-500">Deterministic recommendation · teacher verification required</p></div><div><p className="text-[10px] font-black uppercase tracking-[0.09em] text-slate-500">Evidence source</p><a href={assessment ? `/impact/assessments/${assessment.id}?view=support` : '/impact/assessments'} className="mt-1 block text-sm font-bold text-teal-800 hover:underline">{assessment?.title ?? 'Assessment unavailable'}</a><p className="mt-1 text-xs text-slate-500">{school.name} · {cls?.name}</p></div><div><p className="text-[10px] font-black uppercase tracking-[0.09em] text-slate-500">Topic</p><p className="mt-1 text-sm font-bold text-slate-900">{topic?.name ?? 'Topic not recorded'}</p></div><div className="flex gap-2 lg:flex-col lg:items-end"><StatusBadge tone={item.status === 'completed' ? 'success' : 'info'}>{item.status.replace('_', ' ')}</StatusBadge><StatusBadge tone={item.severity === 'critical' || item.severity === 'high' ? 'attention' : 'neutral'}>{item.severity} priority</StatusBadge></div></article> })}</div></section>)}</div>}
  </div>
}
