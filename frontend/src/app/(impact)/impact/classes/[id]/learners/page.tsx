...
export default function ClassLearnersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <SectionCard title="Class learner evidence" description={`${learners.length} anonymous learner codes · ${assessments.length} recent assessments`}>
      {learners.length === 0 ? <ProductState type="empty" title="No learner codes yet" description="Add an anonymous code such as L001 to begin assessment participation tracking." /> : <div className="overflow-x-auto rounded-xl border border-[#1e293b]"><table className="w-full min-w-[720px] border-collapse text-left"><thead className="bg-white/[0.04]"><tr>{['Learner code', 'Status', 'Assessment participation', 'Support indicator', 'Required action'].map((label) => <th key={label} className="px-4 py-3 text-[11px] font-black uppercase tracking-[0.09em] text-slate-400">{label}</th>)}</tr></thead><tbody className="divide-y divide-[#1e293b]">{learners.map((learner) => { const needsReview = supportIds.has(learner.id); return <tr key={learner.id} className="hover:bg-white/[0.04]"><td className="px-4 py-3 font-mono text-sm font-black text-white">{learner.learner_code}</td><td className="px-4 py-3"><StatusBadge tone="success">{learner.status}</StatusBadge></td><td className="px-4 py-3 text-sm text-slate-300">{assessments.length} of {assessments.length} seeded assessments</td><td className="px-4 py-3"><StatusBadge tone={needsReview ? 'attention' : 'neutral'}>{needsReview ? 'Support indicator' : 'No current indicator'}</StatusBadge></td><td className="px-4 py-3 text-sm font-semibold text-slate-300">{needsReview ? 'Needs teacher review' : 'Continue monitoring'}</td></tr> })}</tbody></table></div>}
    </SectionCard>

    {open && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm"><button type="button" aria-label="Close add learner dialog backdrop" onClick={() => setOpen(false)} className="absolute inset-0" /><section role="dialog" aria-modal="true" aria-labelledby="add-learner-title" className="relative w-full max-w-md rounded-2xl bg-[#0E1626] p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><h2 id="add-learner-title" className="text-xl font-black text-white">Add learner code</h2><p className="mt-2 text-sm leading-6 text-slate-400">Use an anonymous school code. Do not enter a learner name.</p></div><button type="button" aria-label="Close add learner dialog" onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-xl border border-[#1e293b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"><X aria-hidden="true" className="h-4 w-4" /></button></div><form onSubmit={submit} className="mt-6"><label htmlFor="learner-code" className="text-sm font-bold text-slate-200">Learner code</label><input id="learner-code" value={learnerCode} onChange={(event) => setLearnerCode(event.target.value)} placeholder="e.g. L031" required autoFocus className="mt-2 min-h-11 w-full rounded-xl border border-[#1e293b] px-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" /><p className="mt-2 text-xs text-slate-400">Teacher verification required before linking this code to assessment evidence.</p><div className="mt-6 flex gap-3"><button type="button" onClick={() => setOpen(false)} className="min-h-11 flex-1 rounded-xl border border-[#1e293b] px-4 text-sm font-bold text-slate-300">Cancel</button><button type="submit" disabled={createLearner.isPending || !learnerCode.trim()} className="min-h-11 flex-1 rounded-xl bg-[#0b4f5c] px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{createLearner.isPending ? 'Adding…' : 'Add code'}</button></div></form></section></div>}
  </div>
}
