...
export default function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      <SectionCard title="Recent assessments" description="Open an assessment to review questions, marks, results and support indicators."><div className="space-y-3">{assessments.map((item) => <a key={item.id} href={`/impact/assessments/${item.id}`} className="flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-4 hover:border-teal-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"><div className="min-w-0"><p className="truncate text-sm font-extrabold text-slate-900">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.date_written} · {item.total_marks} marks</p></div><StatusBadge tone="success">{item.status}</StatusBadge></a>)}</div></SectionCard>
      <SectionCard title="Topic-performance summary" description="Observed topics needing attention across this class's seeded assessments."><div className="space-y-3">{[...topics.entries()].map(([topicId, topic]) => <div key={topicId} className="flex items-center justify-between gap-4 rounded-xl bg-amber-500/10 p-4"><div><p className="text-sm font-extrabold text-slate-900">{topic.name || SEEDED_TOPICS.topics.find((item) => item.id === topicId)?.name}</p><p className="mt-1 text-xs text-slate-500">Additional support may be helpful</p></div><StatusBadge tone="attention">{topic.percentage}%</StatusBadge></div>)}</div></SectionCard>
    </div>
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      <SectionCard title="Learner-code preview" description="No learner names are used in this demonstration."><div className="flex flex-wrap gap-2">{learners.slice(0, 12).map((learner) => <span key={learner.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-xs font-bold text-slate-700">{learner.learner_code}</span>)}</div><a href={getLearnerRoute(id)} className="mt-4 inline-flex min-h-11 items-center text-sm font-bold text-cyan-700 hover:underline">Open all learner codes →</a></SectionCard>
      <SectionCard title="Active interventions" description="Deterministic support actions; teacher verification required."><div className="space-y-3">{interventions.filter((item) => item.status !== 'completed').map((item) => <div key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start gap-3"><GraduationCap aria-hidden="true" className="mt-0.5 h-4 w-4 text-cyan-700" /><div><p className="text-sm font-bold text-slate-900">{item.recommendation}</p><p className="mt-1 text-xs text-slate-500">{item.status.replace('_', ' ')} · {item.severity} priority</p></div></div></div>)}</div></SectionCard>
    </div>
  </div>
}
