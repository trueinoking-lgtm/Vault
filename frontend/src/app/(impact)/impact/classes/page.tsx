  ...
  return <div><PageHeader eyebrow="Class evidence" title="Classes" description="Every class is associated with a school and opens a real class workflow for learners, topics, assessments and interventions." />
    {classes.length === 0 ? <ProductState type="empty" title="No classes available" description="Classes appear once a school cohort has been created." /> : <div className="space-y-3">
      {classes.map((cls) => {
        ...
        return <a key={cls.id} href={getClassRoute(cls.id)} className="group grid gap-4 rounded-2xl border border-[#1e293b] bg-[#0E1626] p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition hover:border-teal-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 md:grid-cols-[1.4fr_.8fr_.8fr_.8fr_auto] md:items-center">
          <div className="min-w-0"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-cyan-500/10 text-cyan-300"><GraduationCap aria-hidden="true" className="h-5 w-5" /></span><div className="min-w-0"><h2 className="truncate text-base font-black text-white">{cls.name}</h2><p className="mt-1 truncate text-xs font-semibold text-slate-400">{school?.name ?? 'School not recorded'}</p></div></div></div>
          <div><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Teacher</p><p className="mt-1 flex items-center gap-1.5 text-sm font-semibold text-white"><UserRound aria-hidden="true" className="h-3.5 w-3.5 text-slate-400" />{cls.teacher_name || 'Not recorded'}</p><p className="mt-1 text-xs text-slate-400">{cls.academic_year || 'Year not recorded'}</p></div>
          <div><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Learners</p><p className="mt-1 text-lg font-black text-white">{learnerCount}</p><p className="text-xs text-slate-400">Anonymous codes</p></div>
          <div><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-400">Recent assessment</p>{recent ? <><p className="mt-1 truncate text-sm font-bold text-white">{recent.title}</p><p className="mt-1 flex items-center gap-1 text-xs text-slate-400"><CalendarDays aria-hidden="true" className="h-3 w-3" />{recent.date_written}</p></> : <p className="mt-1 text-sm text-slate-400">No recent assessment</p>}</div>
          <div className="flex items-center justify-between gap-3 md:flex-col md:items-end"><StatusBadge tone={rate >= 60 ? 'success' : 'attention'}>{rate}% pass rate</StatusBadge><span className="inline-flex items-center gap-1 text-xs font-black text-cyan-300">Open class <ArrowRight aria-hidden="true" className="h-4 w-4 transition group-hover:translate-x-0.5" /></span></div>
        </a>
      })}
    </div>}
  </div>
