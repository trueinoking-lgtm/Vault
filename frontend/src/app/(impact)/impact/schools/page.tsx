      ...
      {schools.length === 0 ? <ProductState type="empty" title="No schools available" description="Schools appear here once assessment evidence has been connected." /> : (
        <div className="grid gap-5 xl:grid-cols-3">
          {schools.map((school) => {
            ...
            return (
              <article key={school.id} className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(2,6,23,0.06)]">
                <div className="border-b border-slate-200 p-5 sm:p-6">
                  <div className="flex items-start justify-between gap-4"><span className="grid h-11 w-11 place-items-center rounded-xl bg-cyan-500/10 text-cyan-700"><Building2 aria-hidden="true" className="h-5 w-5" /></span><StatusBadge tone={rate >= 60 ? 'success' : 'attention'}>{rate >= 60 ? 'Monitoring' : 'Needs teacher review'}</StatusBadge></div>
                ...
                <div className="mt-auto p-5 sm:p-6"><a href={getSchoolRoute(school.id)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-extrabold text-primary-foreground shadow-sm hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500">Open school <ArrowRight aria-hidden="true" className="h-4 w-4" /></a></div>
              </article>
            )
          })}
        </div>
