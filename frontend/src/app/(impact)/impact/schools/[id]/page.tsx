    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
        <SectionCard title="Topics needing attention" description="Observed assessment performance, not a causal diagnosis.">
          <div className="space-y-3">{dashboard.weakest_topics.map((topic) => <div key={topic.topic_id} className="flex flex-col gap-3 rounded-xl bg-amber-500/10 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-extrabold text-slate-900">{topic.topic_name}</p><p className="mt-1 text-xs text-slate-500">Mapped assessment topic · {topic.num_questions} mapped questions</p></div><StatusBadge tone="attention">{topic.percentage}% performance · teacher review</StatusBadge></div>)}</div>
        </SectionCard>
