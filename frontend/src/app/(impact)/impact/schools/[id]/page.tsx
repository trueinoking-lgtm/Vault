    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
        <SectionCard title="Topics needing attention" description="Observed assessment performance, not a causal diagnosis.">
          <div className="space-y-3">{dashboard.weakest_topics.map((topic) => <div key={topic.topic_id} className="flex flex-col gap-3 rounded-xl bg-[var(--accent-warning)]/10 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-extrabold text-[var(--text-primary)]">{topic.topic_name}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">Mapped assessment topic · {topic.num_questions} mapped questions</p></div><StatusBadge tone="attention">{topic.percentage}% performance · teacher review</StatusBadge></div>)}</div>
        </SectionCard>
        <SectionCard title="Data readiness" description="A transparent indicator of what this seeded school record contains.">
          <div className="space-y-4"><ProgressBar label="Class coverage" value={100} /><ProgressBar label="Assessment mapping" value={100} /><ProgressBar label="Anonymous learner codes" value={100} /><p className="rounded-xl border border-[var(--accent-primary)]/20 bg-[var(--accent-primary)]/10 p-4 text-xs leading-5 text-[var(--accent-primary)]">Ready for product demonstration only. Pilot ingestion, correction workflows and longitudinal evidence still require real school implementation.</p></div>
        </SectionCard>
      </div>
