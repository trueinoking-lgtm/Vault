...
export default function AssessmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-x-auto border-b border-[#1e293b]" role="tablist" aria-label="Assessment workflow"><div className="flex min-w-max gap-1">{TABS.map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} aria-controls={`panel-${item.id}`} onClick={() => selectTab(item.id)} className={`min-h-11 rounded-t-xl border-b-2 px-4 py-3 text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 ${tab === item.id ? 'border-teal-700 bg-[#0E1626] text-cyan-300' : 'border-transparent text-slate-400 hover:bg-[#0E1626]/60 hover:text-white'}`}>{item.label}</button>)}</div></div>
    ...
    <section id={`panel-${tab}`} role="tabpanel" className="min-w-0">
      ...
      {tab === 'report' && <SectionCard title="Print-ready assessment report" description="The report keeps deterministic metrics, limitations and the seeded-data disclosure together."><div className="rounded-2xl bg-white/[0.04] p-6"><FileText aria-hidden="true" className="h-8 w-8 text-cyan-300" /><h2 className="mt-4 text-xl font-black text-white">Assessment evidence report</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-400">Review question performance, topic evidence, support indicators and deterministic interventions in a print-friendly format.</p><a href={`/impact/assessments/${id}/report`} className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-[#0b4f5c] px-4 py-2 text-sm font-bold text-white">View and print report</a></div></SectionCard>}
    </section>
  </div>
}
...
function Results({ analytics, loading, assessmentId }: { analytics: AssessmentAnalytics | undefined; loading: boolean; assessmentId: string }) {
  ...
  return <div className="space-y-5"><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Class average" value={`${analytics.class_average_percentage}%`} detail="Across assessed learners" icon={BarChart3} /><MetricCard label="Pass rate" value={`${analytics.pass_rate}%`} detail="Against the set pass mark" icon={ClipboardCheck} tone="blue" /><MetricCard label="Questions" value={analytics.question_performance.length} detail="Performance rows" icon={ListChecks} tone="violet" /><MetricCard label="Support indicators" value={analytics.at_risk_learners.length} detail="Teacher review required" icon={UsersRound} tone="amber" /></div>
    <div className="grid gap-5 xl:grid-cols-2"><SectionCard title="Topic performance" description="Every mapped topic, coloured from critical (red) to secure (teal). The percentage is performance; the q count shows evidence size.">
      ...
      <div className="space-y-3">{analytics.question_performance.map((q: QuestionPerformance) => <div key={q.question_id} className="flex items-center gap-3"><div className="w-12 shrink-0 text-sm font-bold text-white">Q{q.question_number}</div><div className="flex-1"><BandBar label={q.label ?? `Question ${q.question_number}`} value={q.average_percentage} count={q.num_learners} /></div><div className="shrink-0"><StatusBadge tone={q.is_critical ? 'attention' : 'success'}>{q.is_critical ? 'Review' : 'OK'}</StatusBadge></div></div>)}</div>
    </SectionCard></div>
    <details className="group rounded-2xl border border-[#1e293b] bg-[#0E1626] shadow-sm">
      <summary className="cursor-pointer list-none px-5 py-4 text-sm font-extrabold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-500">Learner-support distribution <span aria-hidden="true" className="float-right text-slate-400 group-open:rotate-180">⌄</span><span className="mt-1 block text-xs font-medium leading-5 text-slate-400">Anonymous learner codes grouped by risk level. Counts are shown — colour alone never drives a decision.</span></summary>
      <div className="border-t border-[#1e293b] p-5">
      <RiskDonut centerLabel={`${analytics.learners_assessed}`} centerSub="learners" segments={[
        { label: 'Low risk', value: analytics.learner_performance.filter((l: LearnerPerformance) => l.risk_level === 'low').length, color: '#0f766e' },
        { label: 'Medium risk', value: analytics.learner_performance.filter((l: LearnerPerformance) => l.risk_level === 'medium').length, color: '#d97706' },
        { label: 'High risk', value: analytics.learner_performance.filter((l: LearnerPerformance) => l.risk_level === 'high').length, color: '#dc2626' },
      ]} />
      <p className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-xs leading-5 text-amber-300">Every indicator requires teacher verification before any action. These distributions are observational, not a judgement on any learner.</p>
      </div>
    </details>
    <details className="group rounded-2xl border border-[#1e293b] bg-[#0E1626] shadow-sm">
      <summary className="cursor-pointer list-none px-5 py-4 text-sm font-extrabold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-500">Optional AI explanation <span aria-hidden="true" className="float-right text-slate-400 group-open:rotate-180">⌄</span><span className="mt-1 block text-xs font-medium leading-5 text-slate-400">Secondary, manually generated content only. Deterministic metrics above remain the source of truth.</span></summary>
      <div className="border-t border-[#1e293b] p-5"><AISummaryPanel assessmentId={assessmentId} /></div>
    </details>
  </div>
}
...
function Support({ analytics, loading }: { analytics: AssessmentAnalytics | undefined; loading: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-2"><SectionCard title="Learner-support signals" description="Anonymous codes only. Every indicator requires teacher verification."><div className="space-y-3">{analytics.at_risk_learners.map((learner: LearnerPerformance) => <div key={learner.learner_id} className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"><div className="flex items-center justify-between gap-4"><div><p className="font-mono text-sm font-black text-white">{learner.learner_code}</p><p className="mt-1 text-xs text-slate-400">{learner.percentage}% assessment result</p></div><StatusBadge tone="attention">Needs teacher review</StatusBadge></div><p className="mt-3 text-xs font-semibold text-amber-300">Additional support may be helpful. Teacher verification required.</p></div>)}</div></SectionCard><SectionCard title="Deterministic interventions" description="Rule-based suggestions derived from topic evidence; teachers decide what to apply."><div className="space-y-3">{analytics.interventions.map((item: InterventionRecommendation, index: number) => <div key={`${item.entity_id}-${index}`} className="rounded-xl border border-[#1e293b] p-4"><div className="flex items-start gap-3"><Target aria-hidden="true" className="mt-0.5 h-4 w-4 text-cyan-300" /><div><p className="text-sm font-bold text-white">{item.recommendation}</p><p className="mt-1 text-xs text-slate-400">Teacher verification required</p></div></div></div>)}</div></SectionCard></div>
}
