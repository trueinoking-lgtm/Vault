...
function Results({ analytics, loading, assessmentId }: { analytics: AssessmentAnalytics | undefined; loading: boolean; assessmentId: string }) {
  ...
  return <div className="space-y-5"><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Class average" value={`${analytics.class_average_percentage}%`} detail="Across assessed learners" icon={BarChart3} /><MetricCard label="Pass rate" value={`${analytics.pass_rate}%`} detail="Against the set pass mark" icon={ClipboardCheck} tone="blue" /><MetricCard label="Questions" value={analytics.question_performance.length} detail="Performance rows" icon={ListChecks} tone="violet" /><MetricCard label="Support indicators" value={analytics.at_risk_learners.length} detail="Teacher review required" icon={UsersRound} tone="amber" /></div>
    <div className="grid gap-5 xl:grid-cols-2"><SectionCard title="Topic performance" description="Every mapped topic, coloured from critical (red) to secure (teal). The percentage is performance; the q count shows evidence size.">
      ...
      <div className="space-y-3">{analytics.question_performance.map((q: QuestionPerformance) => <div key={q.question_id} className="flex items-center gap-3"><div className="w-12 shrink-0 text-sm font-bold text-slate-950">Q{q.question_number}</div><div className="flex-1"><BandBar label={q.label ?? `Question ${q.question_number}`} value={q.average_percentage} count={q.num_learners} /></div><div className="shrink-0"><StatusBadge tone={q.is_critical ? 'attention' : 'success'}>{q.is_critical ? 'Review' : 'OK'}</StatusBadge></div></div>)}</div>
    </SectionCard></div>
    <details className="group rounded-2xl border border-slate-200 bg-white shadow-sm">
      <summary className="cursor-pointer list-none px-5 py-4 text-sm font-extrabold text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-500">Learner-support distribution <span aria-hidden="true" className="float-right text-slate-400 group-open:rotate-180">⌄</span><span className="mt-1 block text-xs font-medium leading-5 text-slate-500">Anonymous learner codes grouped by risk level. Counts are shown — colour alone never drives a decision.</span></summary>
      ...
      <p className="mt-4 rounded-xl border border-amber-100 bg-amber-50 p-4 text-xs leading-5 text-amber-950">Every indicator requires teacher verification before any action. These distributions are observational, not a judgement on any learner.</p>
      </div>
    </details>
    <details className="group rounded-2xl border border-slate-200 bg-white shadow-sm">
      <summary className="cursor-pointer list-none px-5 py-4 text-sm font-extrabold text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-500">Optional AI explanation <span aria-hidden="true" className="float-right text-slate-400 group-open:rotate-180">⌄</span><span className="mt-1 block text-xs font-medium leading-5 text-slate-500">Secondary, manually generated content only. Deterministic metrics above remain the source of truth.</span></summary>
      <div className="border-t border-slate-100 p-5"><AISummaryPanel assessmentId={assessmentId} /></div>
    </details>
  </div>
}
