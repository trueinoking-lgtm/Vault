...
export default function AssessmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-x-auto border-b border-slate-200" role="tablist" aria-label="Assessment workflow"><div className="flex min-w-max gap-1">{TABS.map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} aria-controls={`panel-${item.id}`} onClick={() => selectTab(item.id)} className={`min-h-11 rounded-t-xl border-b-2 px-4 py-3 text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 ${tab === item.id ? 'border-teal-700 bg-white text-cyan-700' : 'border-transparent text-slate-500 hover:bg-white/60 hover:text-slate-900'}`}>{item.label}</button>)}</div></div>
