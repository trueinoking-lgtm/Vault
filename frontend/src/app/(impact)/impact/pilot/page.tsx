...
export default function PilotModePage() {
  ...
  return <div className="space-y-8 pb-12">
    ...
    <section className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-5" aria-label="Pilot access boundary"><div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-cyan-700" aria-hidden="true" /><div><h2 className="font-extrabold text-cyan-700">Physically separated pilot dataset</h2><p className="mt-1 text-sm leading-6 text-cyan-700/80">Pilot APIs read only <code>pilot_*</code> records. Demo dashboards continue to read the canonical seeded environment. Authenticated workspace membership limits school access.</p></div></div></section>
    <Message message={message} />

    {loading ? <div role="status" className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm font-semibold text-slate-500">Checking pilot access…</div> : null}
