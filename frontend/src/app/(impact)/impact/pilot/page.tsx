...
const fieldClass = 'min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200'
...
const secondaryButton = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500'

function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string }) {
  return <label className="block text-sm font-bold text-slate-700"><span>{label}</span>{hint && <span className="ml-2 text-xs font-medium text-slate-500">{hint}</span>}<span className="mt-1.5 block">{children}</span></label>
}

function Message({ message }: { message: { type: 'success' | 'error'; text: string } | null }) {
  if (!message) return null
  return <div role={message.type === 'error' ? 'alert' : 'status'} className={`mt-4 rounded-xl border px-4 py-3 text-sm font-semibold ${message.type === 'error' ? 'border-rose-500/30 bg-rose-500/10 text-rose-300' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700'}`}>{message.text}</div>
}
...
function Preview({ preview, onConfirm, busy }: { preview: ImportPreview | null; onConfirm: () => void; busy: boolean }) {
  ...
  return <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
    ...
    <div className="mt-3 max-h-64 overflow-auto rounded-lg border border-slate-200 bg-white">
      <table className="w-full min-w-[560px] text-left text-xs"><thead className="sticky top-0 bg-slate-200 text-slate-500"><tr><th className="p-2">Row</th><th className="p-2">Data</th><th className="p-2">Validation</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.row_number} className="border-t border-slate-200"><td className="p-2 font-bold">{row.row_number}</td><td className="p-2 font-mono">{Object.values(row.data).join(' · ')}</td><td className={`p-2 font-semibold ${row.accepted ? 'text-emerald-700' : 'text-rose-300'}`}>{row.accepted ? 'Accepted' : row.errors.join(' ')}</td></tr>)}</tbody></table>
    </div>
...
export default function PilotModePage() {
  ...
  return <div className="space-y-8 pb-12">
    ...
    <section className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-5" aria-label="Pilot access boundary"><div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-cyan-700" aria-hidden="true" /><div><h2 className="font-extrabold text-cyan-700">Physically separated pilot dataset</h2><p className="mt-1 text-sm leading-6 text-cyan-700/80">Pilot APIs read only <code>pilot_*</code> records. Demo dashboards continue to read the canonical seeded environment. Authenticated workspace membership limits school access.</p></div></div></section>
    <Message message={message} />

    {loading ? <div role="status" className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm font-semibold text-slate-500">Checking pilot access…</div> : null}
