...
const fieldClass = 'min-h-11 w-full rounded-xl border border-[#1e293b] bg-[#0E1626] px-3 py-2 text-sm text-white shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200'
...
const secondaryButton = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#1e293b] bg-[#0E1626] px-4 py-2.5 text-sm font-bold text-slate-300 hover:bg-white/[0.04] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500'

function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string }) {
  return <label className="block text-sm font-bold text-slate-200"><span>{label}</span>{hint && <span className="ml-2 text-xs font-medium text-slate-400">{hint}</span>}<span className="mt-1.5 block">{children}</span></label>
}

function Message({ message }: { message: { type: 'success' | 'error'; text: string } | null }) {
  if (!message) return null
  return <div role={message.type === 'error' ? 'alert' : 'status'} className={`mt-4 rounded-xl border px-4 py-3 text-sm font-semibold ${message.type === 'error' ? 'border-rose-500/30 bg-rose-500/10 text-rose-300' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300'}`}>{message.text}</div>
}

function Preview({ preview, onConfirm, busy }: { preview: ImportPreview | null; onConfirm: () => void; busy: boolean }) {
  if (!preview) return null
  return <div className="mt-5 rounded-xl border border-[#1e293b] bg-white/[0.04] p-4">
    ...
    <div className="mt-3 max-h-64 overflow-auto rounded-lg border border-[#1e293b] bg-[#0E1626]">
      <table className="w-full min-w-[560px] text-left text-xs"><thead className="sticky top-0 bg-white/10 text-slate-400"><tr><th className="p-2">Row</th><th className="p-2">Data</th><th className="p-2">Validation</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.row_number} className="border-t border-[#1e293b]"><td className="p-2 font-bold">{row.row_number}</td><td className="p-2 font-mono">{Object.values(row.data).join(' · ')}</td><td className={`p-2 font-semibold ${row.accepted ? 'text-emerald-300' : 'text-rose-300'}`}>{row.accepted ? 'Accepted' : row.errors.join(' ')}</td></tr>)}</tbody></table>
    </div>
...
export default function PilotModePage() {
  ...
  return <div className="space-y-8 pb-12">
    ...
    <section className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-5" aria-label="Pilot access boundary"><div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-cyan-300" aria-hidden="true" /><div><h2 className="font-extrabold text-cyan-300">Physically separated pilot dataset</h2><p className="mt-1 text-sm leading-6 text-cyan-300/80">Pilot APIs read only <code>pilot_*</code> records. Demo dashboards continue to read the canonical seeded environment. Authenticated workspace membership limits school access.</p></div></div></section>
    <Message message={message} />

    {loading ? <div role="status" className="rounded-2xl border border-[#1e293b] bg-[#0E1626] p-8 text-center text-sm font-semibold text-slate-400">Checking pilot access…</div> : null}
    ...
    {workspace && <>
      ...
      <SectionCard title="9. Qualified before/after comparison" description="Assessment difficulty, topic coverage, and learner participation may differ."><div id="comparison"><button disabled={busy || !followUp} onClick={compare} className={buttonClass}>Compare diagnostic and follow-up</button>{comparison && <div className="mt-5 grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-[#1e293b] p-4"><p className="text-xs font-bold text-slate-400">Average change</p><p className="mt-1 text-2xl font-black">{String(comparison.average_change)} points</p></div><div className="rounded-xl border border-[#1e293b] p-4"><p className="text-xs font-bold text-slate-400">Pass-rate change</p><p className="mt-1 text-2xl font-black">{String(comparison.pass_rate_change)} points</p></div><div className="rounded-xl border border-[#1e293b] p-4"><p className="text-xs font-bold text-slate-400">Participation change</p><p className="mt-1 text-2xl font-black">{String(comparison.participation_change)}</p></div><div className="md:col-span-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"><p className="font-extrabold text-amber-300">Performance changed after the intervention period.</p><p className="mt-2 text-sm leading-6 text-amber-300">{String(comparison.comparability_warning)}</p></div></div>}</div></SectionCard>

      <SectionCard title="10. Pilot evidence report" description="Versioned, print-friendly, teacher-reviewed evidence with explicit non-causal disclosure."><div id="report" className="flex flex-wrap gap-3"><button disabled={busy || !comparison} onClick={generateReport} className={buttonClass}><FileText className="h-4 w-4" />Generate pilot evidence report</button>{report && <button onClick={() => window.print()} className={secondaryButton}><Download className="h-4 w-4" />Print to PDF</button>}</div>{report && <article className="pilot-report mt-6 rounded-2xl border border-[#1e293b] bg-[#0E1626] p-6 shadow-sm"><header className="border-b border-[#1e293b] pb-4"><p className="text-xs font-extrabold uppercase tracking-wider text-cyan-300">ZimLearnGraph controlled pilot evidence</p><h2 className="mt-2 text-2xl font-black">{workspace.school_name}</h2><p className="mt-1 text-sm text-slate-400">Report version {String(report.version)} · {new Date().toLocaleDateString()}</p></header><div className="mt-5 grid gap-5 md:grid-cols-2"><div><h3 className="font-extrabold">1. Pilot context</h3><p className="mt-2 text-sm leading-6">{workspace.academic_year}, {workspace.term}. Participating teacher access is recorded in the audit trail.</p></div><div><h3 className="font-extrabold">2. Diagnostic assessment</h3><p className="mt-2 text-sm leading-6">Average {String((report.snapshot as { diagnostic?: { average?: unknown } })?.diagnostic?.average)}%; pass rate {String((report.snapshot as { diagnostic?: { pass_rate?: unknown } })?.diagnostic?.pass_rate)}%. Support indicators require teacher review.</p></div><div><h3 className="font-extrabold">3. Intervention</h3><p className="mt-2 text-sm leading-6">Suggested by the system. Teacher verification and completion are stored separately.</p></div><div><h3 className="font-extrabold">4. Follow-up assessment</h3><p className="mt-2 text-sm leading-6">Average {String((report.snapshot as { follow_up?: { average?: unknown } })?.follow_up?.average)}%; pass rate {String((report.snapshot as { follow_up?: { pass_rate?: unknown } })?.follow_up?.pass_rate)}%.</p></div><div className="md:col-span-2"><h3 className="font-extrabold">5. Interpretation and limitations</h3><p className="mt-2 text-sm leading-6">Performance changed after the intervention period. Results should be reviewed by the teacher because assessment difficulty and participation may differ. School review status: {String(report.school_review_status)}.</p></div><div className="md:col-span-2 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"><h3 className="font-extrabold text-amber-300">6. Disclosure</h3><p className="mt-2 text-sm leading-6 text-amber-300">{PILOT_REPORT_DISCLOSURE}</p></div></div></article>}</SectionCard>
