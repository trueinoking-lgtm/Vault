...
**Internal**: `vault_core.domain.notebook` (Source, Note, SourceInsight), `vault_core.utils.chunking` (chunk_text, detect_content_type), `vault_core.utils.embedding` (generate_embedding, generate_embeddings), `vault_core.database.repository` (repo_query, repo_insert)
...
```python
@command("process_source", app="vault_core", retry={
    "max_attempts": 5,
