...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with Vault, including different deployment scenarios and network configurations.
...
When both Vault and Ollama run directly on your machine:
...
### Scenario 2: Vault in Docker, Ollama on Host

When Vault runs in Docker but Ollama runs on your host machine:
...
When both Vault and Ollama run in the same Docker Compose stack:
...
# https://github.com/lfnovo/vault/blob/main/docker-compose.yml
services:
  vault:
    image: lfnovo/vault_core:v1-latest
    pull_policy: always
...
This is the most common cause of "Failed to send message" errors. Vault requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in Vault**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in Vault, not just `model`.

**Step 3: Configure in Vault**
...
```bash
# If Vault runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When Vault runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside Vault container, test Ollama:**
```bash
# Get into container
...
1. **Model name mismatch**: The model name in Vault doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update Vault's defaults
4. **Model record deleted**: The model was removed from Vault but is still set as default
...
# Compare with what's configured in Vault
...
docker run --network host lfnovo/vault_core:v1-latest  # for quick testing only
```
...
# Vault configuration (configure via Settings → API Keys instead)
...
- [Vault Discord](https://discord.gg/37XJPXfz2w) - Integration help
...
This comprehensive guide should help you successfully deploy and optimize Ollama with Vault. Start with the Quick Start section and refer to specific scenarios as needed.