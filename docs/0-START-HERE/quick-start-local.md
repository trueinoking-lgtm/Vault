...
Get Vault running with **100% local AI** using Ollama. No cloud API keys needed, completely private.
...
Create a new folder `vault-local` and add this file:
...
Open terminal in your `vault-local` folder:
...
docker exec vault-local-ollama-1 ollama pull neural-chat

# OR: Even better quality, more VRAM needed
docker exec vault-local-ollama-1 ollama pull llama2
```
...
```bash
# Show available GPUs
docker exec vault-local-ollama-1 ollama ps
...
docker exec vault-local-ollama-1 ollama list
...
4. In Vault, go to **Settings** → **API Keys**
