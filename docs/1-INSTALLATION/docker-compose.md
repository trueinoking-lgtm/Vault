...
> **Alternative Registry:** All images are available on both Docker Hub (`lfnovo/vault_core`) and GitHub Container Registry (`ghcr.io/lfnovo/vault`). Use GHCR if Docker Hub is blocked or you prefer GitHub-native workflows.
...
The official `docker-compose.yml` is in the root of our repository: [View on GitHub](https://github.com/lfnovo/vault/blob/main/docker-compose.yml)
...
Open terminal in the `vault` folder:
...
✅ vault_core running on :8502 (UI) and :5055 (API)
```
...
Done! You now have a fully working Vault instance.
...
curl -o docker-compose.yml https://raw.githubusercontent.com/lfnovo/vault/main/examples/docker-compose-ollama.yml
...
docker exec vault-local-ollama-1 ollama pull mistral
```
...
|----------|---------|---------|
| `VAULT_ENCRYPTION_KEY` | Encryption key for credentials | `my-secret-key` |
...
| `SURREAL_NAMESPACE` | Database namespace | `vault_core` |
| `SURREAL_DATABASE` | Database name | `vault_core` |
| `API_URL` | API external URL | `http://localhost:5055` |
| `VAULT_EMBEDDING_BATCH_SIZE` | Override embedding batch size for stricter/local providers (recommended: `8` for CPU-only local setups) | `50` |
