...
```bash
export VAULT_CHUNK_SIZE=1500
export VAULT_CHUNK_OVERLAP=150
```
...
- **CHUNK_SIZE**: Configurable via `VAULT_CHUNK_SIZE` env var (default: 400)
- **CHUNK_OVERLAP**: Configurable via `VAULT_CHUNK_OVERLAP` env var (default: 15% of CHUNK_SIZE)
...
- `vault_core.domain.notebook`: Source, Note, SourceInsight models; vector_search function
- `vault_core.ai.models`: model_manager for embedding model access
- `vault_core.exceptions`: DatabaseOperationError, NotFoundError
...
- **Chunk size semantics changed**: `VAULT_CHUNK_SIZE` and `VAULT_CHUNK_OVERLAP` are token-based, not character-based
...
3. **Change chunk size**: Set VAULT_CHUNK_SIZE and VAULT_CHUNK_OVERLAP environment variables
...
### Chunking
```python
from vault_core.utils.chunking import chunk_text, detect_content_type, ContentType
...
### Embedding
```python
from vault_core.utils.embedding import generate_embedding, generate_embeddings
...
**Key behavior**:
- Key source: VAULT_ENCRYPTION_KEY_FILE (Docker secrets) → VAULT_ENCRYPTION_KEY (env var)
...
- VAULT_ENCRYPTION_KEY must be set explicitly (no default)
...
**Usage Example**:
```python
from vault_core.utils.encryption import encrypt_value, decrypt_value
