...
- Singleton configuration record (record_id: `vault_core:default_models`)
...
- `vault_core.database.repository`: repo_query, ensure_record_id
- `vault_core.domain.base`: ObjectModel, RecordModel base classes
- `vault_core.domain.credential`: Credential for database-stored API keys
- `vault_core.utils`: token_count() for context size detection
