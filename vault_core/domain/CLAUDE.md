...
- **Credential**: Individual credential records for API keys and provider configuration
  ...
  - **Encryption integration**: Uses `encrypt_value()`/`decrypt_value()` from `vault_core.utils.encryption`
    - Keys encrypted with Fernet before database storage
    - Requires `VAULT_ENCRYPTION_KEY` environment variable (warns if not set)
  - **Key methods**:
...
- `vault_core.database.repository`: CRUD and relationship functions
- `vault_core.ai.models`: ModelManager for embeddings
